...
type NetworkSettingsProxyProtocol struct {
	...
	ProxyProtocolTrustedIPs []string `json:"proxyProtocolTrustedIPs"`
}
...
func buildInbound(nodeInfo *panel.NodeInfo, tag string) (*core.InboundHandlerConfig, error) {
	...
	if len(nodeInfo.Common.NetworkSettings) > 0 {
		...
		if n.AcceptProxyProtocol {
			if nodeInfo.Type == "hysteria2" || nodeInfo.Type == "tuic" {
				trusted, validationErr := validateUDPProxyTrustedIPs(n.ProxyProtocolTrustedIPs)
				if validationErr != nil {
					return nil, validationErr
				}
				udpProxyTrustedIPs = trusted
			}
	...
	if len(udpProxyTrustedIPs) > 0 {
		if in.StreamSetting == nil || in.StreamSetting.SocketSettings == nil {
			return nil, errors.New("stream settings must be configured for UDP PROXY protocol")
		}
		// Packet transports do not consume HTTP X-Forwarded-For. The patched
		// Xray PacketConn reads this field exclusively as relay CIDR allow-list.
		in.StreamSetting.SocketSettings.TrustedXForwardedFor = udpProxyTrustedIPs
	}
...
func validateUDPProxyTrustedIPs(values []string) ([]string, error) {
	if len(values) == 0 {
		return nil, errors.New("UDP PROXY protocol requires proxyProtocolTrustedIPs")
	}
	if len(values) > 32 {
		return nil, errors.New("UDP PROXY protocol allows at most 32 trusted relay IPs")
	}
	...
	for _, value := range values {
		...
		if value == "" {
			return nil, errors.New("UDP PROXY protocol trusted relay IP cannot be empty")
		}
		if ip := stdnet.ParseIP(value); ip != nil {
			value = ip.String()
		} else if ip, network, err := stdnet.ParseCIDR(value); err == nil {
			ones, _ := network.Mask.Size()
			if ones == 0 {
				return nil, fmt.Errorf("UDP PROXY protocol trusted relay %q must not be a wildcard CIDR", value)
			}
			value = (&stdnet.IPNet{IP: ip, Mask: network.Mask}).String()
		} else {
			return nil, fmt.Errorf("invalid UDP PROXY protocol trusted relay IP/CIDR: %q", value)
		}
...
func buildHysteria2(nodeInfo *panel.NodeInfo, inbound *coreConf.InboundDetourConfig) error {
	...
	finalmask := &coreConf.FinalMask{
		QuicParams: &coreConf.QuicParamsConfig{
			// Keep the V2Node profile: a 30-second QUIC idle window with a
			// 10-second keepalive keeps carrier NAT mappings warm without
			// allowing dead paths to linger for the upstream default window.
			MaxIdleTimeout:  30,
			KeepAlivePeriod: 10,
		},
	}
