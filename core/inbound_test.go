...
func TestHysteriaUsesV2NodeTimeoutsWithActiveKeepAlive(t *testing.T) {
	inbound := &coreConf.InboundDetourConfig{}
	err := buildHysteria2(&panel.NodeInfo{Common: &panel.CommonNode{
	...
	if inbound.StreamSetting == nil || inbound.StreamSetting.FinalMask == nil ||
		inbound.StreamSetting.FinalMask.QuicParams == nil {
		t.Fatal("missing hysteria2 QUIC parameters")
	}
	quic := inbound.StreamSetting.FinalMask.QuicParams
	if quic.MaxIdleTimeout != 30 || quic.KeepAlivePeriod != 10 {
		t.Fatalf("QUIC timers = idle %ds keepalive %ds, want 30s/10s", quic.MaxIdleTimeout, quic.KeepAlivePeriod)
	}
	if inbound.StreamSetting.HysteriaSettings == nil || inbound.StreamSetting.HysteriaSettings.UdpIdleTimeout != 60 {
		t.Fatalf("Hysteria UDP idle timeout = %#v, want 60s", inbound.StreamSetting.HysteriaSettings)
	}
}

func TestHysteriaBandwidthKeepsStableTimers(t *testing.T) {
	inbound := &coreConf.InboundDetourConfig{}
	err := buildHysteria2(&panel.NodeInfo{Common: &panel.CommonNode{
	...
	quic := inbound.StreamSetting.FinalMask.QuicParams
	if quic.Congestion != "force-brutal" || quic.MaxIdleTimeout != 30 || quic.KeepAlivePeriod != 10 {
		t.Fatalf("bandwidth QUIC parameters lost stable timers: %#v", quic)
	}
}
...
func TestBuildInboundEnablesProxyProtocolOnReceiverSocket(t *testing.T) {
	inbound, err := buildInbound(&panel.NodeInfo{
		Type:     "vless",
		Security: panel.None,
		Common: &panel.CommonNode{
	...
	if receiver.StreamSettings == nil || receiver.StreamSettings.SocketSettings == nil {
		t.Fatal("proxy protocol requires receiver socket settings")
	}
	if !receiver.StreamSettings.SocketSettings.AcceptProxyProtocol {
		t.Fatal("proxy protocol setting was not propagated to the receiver socket")
	}
}

func TestValidateUDPProxyTrustedIPs(t *testing.T) {
	trusted, err := validateUDPProxyTrustedIPs([]string{" 127.0.0.1 ", "2001:db8::/64", "127.0.0.1"})
	if err != nil {
		t.Fatal(err)
	}
	if len(trusted) != 2 || trusted[0] != "127.0.0.1" || trusted[1] != "2001:db8::/64" {
		t.Fatalf("trusted = %#v", trusted)
	}
	if _, err := validateUDPProxyTrustedIPs(nil); err == nil {
		t.Fatal("missing trusted relay list must fail")
	}
	if _, err := validateUDPProxyTrustedIPs([]string{"not-an-ip"}); err == nil {
		t.Fatal("invalid trusted relay must fail")
	}
	if _, err := validateUDPProxyTrustedIPs([]string{"0.0.0.0/0"}); err == nil {
		t.Fatal("wildcard trusted relay must fail")
	}
}

func TestBuildHysteriaInboundPropagatesUDPProxyTrustedRelays(t *testing.T) {
	inbound, err := buildInbound(&panel.NodeInfo{
		Type:     "hysteria2",
		Security: panel.None,
		Common: &panel.CommonNode{
			ListenIP:                "0.0.0.0",
			ServerPort:              443,
			Network:                 "tcp", // panel placeholder; Hysteria builder selects UDP/QUIC.
			Ignore_Client_Bandwidth: true,
			NetworkSettings: json.RawMessage(`{
				"acceptProxyProtocol": true,
				"proxyProtocolTrustedIPs": ["203.0.113.8", "2001:db8:1::/64"]
			}`),
		},
	}, "hysteria-udp-proxy")
	if err != nil {
	...
	socket := receiver.GetStreamSettings().GetSocketSettings()
	if socket == nil || !socket.AcceptProxyProtocol {
		t.Fatal("Hysteria2 UDP PROXY protocol was not enabled on the receiver socket")
	}
	want := []string{"203.0.113.8", "2001:db8:1::/64"}
	if got := socket.TrustedXForwardedFor; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("trusted UDP relays = %#v, want %#v", got, want)
	}
}
