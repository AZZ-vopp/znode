...
func ConfigureSessionLimits(perUser, global int) {
	if perUser < 1 {
		perUser = defaultMaxConnectionsPerUser
	}
	if global < perUser {
		global = perUser
	}
...
func (d *DefaultDispatcher) Init(config *Config, om outbound.Manager, router routing.Router, pm policy.Manager, sm stats.Manager) error {
	...
	if d.maxConnectionsPerUser < 1 {
		d.maxConnectionsPerUser = defaultMaxConnectionsPerUser
	}
...
func (d *DefaultDispatcher) getLink(ctx context.Context) (*transport.Link, *transport.Link, *limiter.Limiter, error) {
	...
	if user != nil && len(user.Email) > 0 {
		...
		shutdownPeer := &managedShutdownPeer{writer: downlinkWriter}
		managedWriter := &ManagedWriter{writer: uplinkWriter, shutdownPeer: shutdownPeer}
		...
		outboundLink.Writer = &trafficWriter{counter: &ts.DownCounter, manager: manager, writer: shutdownPeer}
		if w != nil {
			sessionInbound.CanSpliceCopy = 3
			inboundLink.Writer = rate.NewRateLimitWriter(inboundLink.Writer, w)
			outboundLink.Writer = rate.NewRateLimitWriter(outboundLink.Writer, w)
		}
		touch := prepareDeviceTouch(limit, user.Email, sessionInbound.Source.Address.IP().String())
...
func (d *DefaultDispatcher) DispatchLink(ctx context.Context, destination net.Destination, outbound *transport.Link) error {
	...
	if user != nil && len(user.Email) > 0 {
		...
		managedWriter := &ManagedWriter{writer: outbound.Writer}
		terminalReader := outbound.Reader
		var shutdownPeer *managedShutdownPeer
		if sessionInbound.Conn != nil {
			// Keep the raw inbound connection as a terminal target. Do not also put
			// outbound.Writer here: the managed primary owns that close, and a
			// second close can be unsafe for protocol writers that are not idempotent.
			shutdownPeer = &managedShutdownPeer{target: sessionInbound.Conn}
			managedWriter.shutdownPeer = shutdownPeer
			terminalReader = &managedTerminalReader{reader: outbound.Reader, shutdownPeer: shutdownPeer}
		}
		manager := d.addManagedLink(user.Email, managedWriter, terminalReader)
		if manager == nil {
			common.Close(outbound.Writer)
			common.Interrupt(outbound.Reader)
		...
		outbound.Reader = newManagedTimeoutReader(terminalReader, &ts.UpCounter, manager)
	}
