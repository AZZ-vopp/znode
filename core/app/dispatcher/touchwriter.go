...
func prepareDeviceTouch(limit *limiter.Limiter, userTag, rawIP string) func() bool {
	deviceIP, excluded := limit.PrepareDeviceIP(rawIP)
	return func() bool {
		if excluded {
			return true
		}
		return limit.TouchPreparedDevice(userTag, deviceIP)
	}
}
