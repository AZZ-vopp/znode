...
type AgentManifest struct {
	...
	UDPRelayRevision         string                         `json:"udp_relay_revision,omitempty"`
	UDPRelays                []UDPRelay                     `json:"udp_relays,omitempty"`
...
// UDPRelay is a panel-managed relay hosted by this Agent.  It is separate
// from Nodes because a relay-only VPS must not create an Xray inbound.
...
func newAgentHTTPClient(c conf.AgentConfig, retryCount int) *resty.Client {
	...
	client := resty.New().
		...
		SetHeader("X-ZNode-Capabilities", "update_geodata,default_wg_balancer").
...
func (c *AgentClient) GetManifest(ctx context.Context) (*AgentManifest, error) {
	...
	if response.IsError() {
		return nil, &controlPlaneHTTPError{operation: "get agent manifest", status: response.StatusCode()}
	}
...
func (m *AgentManifest) Validate() error {
	...
	if len(m.NodeRevision) > 256 || len(m.FallbackRevision) > 256 || len(m.UDPRelayRevision) > 256 {
		return fmt.Errorf("invalid agent manifest: component revision is too long")
	}
	if len(m.UDPRelays) > maxAgentUDPRelays {
		return fmt.Errorf("invalid agent manifest: too many UDP relays")
	}
	relayIDs := make(map[string]struct{}, len(m.UDPRelays))
	for _, relay := range m.UDPRelays {
		if len(relay.ID) == 0 || len(relay.ID) > 128 {
			return fmt.Errorf("invalid UDP relay ID")
		}
		if _, ok := relayIDs[relay.ID]; ok {
			return fmt.Errorf("invalid agent manifest: duplicate UDP relay ID %q", relay.ID)
		}
		relayIDs[relay.ID] = struct{}{}
		if relay.Network != "udp4" && relay.Network != "udp6" || relay.TTLSeconds < 1 || relay.TTLSeconds > 86400 || relay.MaxFlows < 1 || relay.MaxFlows > 100000 {
			return fmt.Errorf("invalid UDP relay %q", relay.ID)
		}
		if !validUDPRelayEndpoint(relay.Listen, true) || !validUDPRelayEndpoint(relay.Upstream, false) {
			return fmt.Errorf("invalid UDP relay endpoint %q", relay.ID)
		}
	}
...
func validUDPRelayEndpoint(value string, allowEmptyHost bool) bool {
	if len(value) == 0 || len(value) > 255 {
		return false
	}
	if allowEmptyHost && strings.HasPrefix(value, ":") {
...
func (m *AgentManifest) EffectiveUDPRelayRevision() string {
	if revision := strings.TrimSpace(m.UDPRelayRevision); revision != "" {
		return revision
	}
	payload, _ := json.Marshal(m.UDPRelays)
	digest := sha256.Sum256(payload)
	return hex.EncodeToString(digest[:])
}
...
func (m *AgentManifest) NodeConfigs(agent conf.AgentConfig) []conf.NodeConfig {
	...
	// Component-aware ZBoard manifests are authoritative for Redis. A null
	// value means the panel has no usable shared Redis endpoint, so falling back
	// to a partial local Agent block (often Enable=true with no address) must not
	// silently dial 127.0.0.1:6379. Preserve the local fallback only for legacy
	// panels that do not publish component revisions.
	if (m.NodeRevision != "" && m.FallbackRevision != "") || m.GlobalDeviceLimitConfig != nil {
