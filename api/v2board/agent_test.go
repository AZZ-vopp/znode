...
func TestAgentClientGetsManifestWithCredentials(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("X-ZNode-Capabilities"); got != "update_geodata,default_wg_balancer" {
	...
	if nodes[0].GlobalDeviceLimitConfig != nil || nodes[1].GlobalDeviceLimitConfig != nil {
		t.Fatalf("modern manifest null Redis config was not authoritative: %+v", nodes)
	}
}

func TestLegacyAgentManifestKeepsLocalRedisFallback(t *testing.T) {
	deviceLimit := &conf.GlobalDeviceLimitConfig{Enable: true, RedisAddr: "redis.example:6379"}
	manifest := AgentManifest{PanelType: conf.RequiredPanelType, Revision: "legacy", Nodes: []int{12, 18}}
	nodes := manifest.NodeConfigs(conf.AgentConfig{
		APIHost: "https://panel.example", AgentID: "agent-123", AgentToken: "top-secret",
		GlobalDeviceLimitConfig: deviceLimit,
	})
	if nodes[0].GlobalDeviceLimitConfig == nil || nodes[1].GlobalDeviceLimitConfig == nil ||
		nodes[0].GlobalDeviceLimitConfig.RedisAddr != "redis.example:6379" {
		t.Fatalf("legacy Agent Redis fallback was not preserved: %+v", nodes)
	}
