...
func TestTrafficReportDistinguishesProcessingConflict(t *testing.T) {
	const reportID = "0123456789abcdef0123456789abcdef"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		if advertiseTrafficProtocol2(w, request) {
			return
		...
		_, _ = w.Write([]byte(`{"data":false,"report_id":"` + reportID + `","status":"processing","code":"traffic_report_processing"}`))
	}))
	defer server.Close()
	...
	err = client.ReportUserTraffic(context.Background(), reportID, []UserTraffic{{UID: 7, Upload: 1}})
	var processingError *TrafficReportProcessingError
	if !errors.As(err, &processingError) {
		t.Fatalf("processing 409 error = %T %v, want TrafficReportProcessingError", err, err)
	}
	if processingError.RetryAfter != 15*time.Second {
		t.Fatalf("retry after = %s, want 15s", processingError.RetryAfter)
	}
}

func TestTrafficReportDistinguishesPayloadConflict(t *testing.T) {
	const reportID = "0123456789abcdef0123456789abcdef"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		if advertiseTrafficProtocol2(w, request) {
			return
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusConflict)
		_, _ = w.Write([]byte(`{"error":"Traffic report payload changed","code":"traffic_report_payload_changed"}`))
	}))
	defer server.Close()
	...
	err = client.ReportUserTraffic(context.Background(), reportID, []UserTraffic{{UID: 7, Upload: 1}})
	var conflictError *TrafficReportPayloadConflictError
	if !errors.As(err, &conflictError) {
		t.Fatalf("payload conflict error = %T %v, want TrafficReportPayloadConflictError", err, err)
	}
}

func TestTrafficReportDoesNotTrustProcessingForAnotherReport(t *testing.T) {
	const reportID = "0123456789abcdef0123456789abcdef"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		if advertiseTrafficProtocol2(w, request) {
			return
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusConflict)
		_, _ = w.Write([]byte(`{"data":false,"report_id":"ffffffffffffffffffffffffffffffff","status":"processing","code":"traffic_report_processing"}`))
	}))
	defer server.Close()
	...
	err = client.ReportUserTraffic(context.Background(), reportID, []UserTraffic{{UID: 7, Upload: 1}})
	var processingError *TrafficReportProcessingError
	if err == nil || errors.As(err, &processingError) {
		t.Fatalf("mismatched processing response error = %T %v", err, err)
	}
}
