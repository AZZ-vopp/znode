...
func (c *Client) GetUserAlive(ctx context.Context) (map[int]int, error) {
	...
	if r.StatusCode() < http.StatusOK || r.StatusCode() >= http.StatusMultipleChoices {
		return nil, &controlPlaneHTTPError{operation: "get user alive list", status: r.StatusCode()}
	}
	defer r.RawResponse.Body.Close()
	next := &AliveMap{}
...
// TrafficReportProcessingError means the panel has durably claimed the
// immutable report but has not committed every accounting update yet. The
// caller must retain and retry the same batch and report ID.
type TrafficReportProcessingError struct {
	ReportID   string
	RetryAfter time.Duration
}

func (e *TrafficReportProcessingError) Error() string {
	return "report user traffic: panel is still processing the immutable batch"
}

// TrafficReportPayloadConflictError means a report ID was reused with a
// different body. Retrying remains safe but cannot resolve the conflict, so it
// must be visible separately from an ordinary processing response.
type TrafficReportPayloadConflictError struct {
	ReportID string
}

func (e *TrafficReportPayloadConflictError) Error() string {
	return "report user traffic: panel rejected a changed immutable batch"
}
...
func (c *Client) ReportUserTraffic(ctx context.Context, reportID string, userTraffic []UserTraffic) error {
	...
	if response == nil || response.StatusCode() < http.StatusOK || response.StatusCode() >= http.StatusMultipleChoices {
		...
		if response.StatusCode() == http.StatusConflict && reportID != "" {
			var body trafficReportResponse
			if err := json.Unmarshal(response.Body(), &body); err == nil {
				if body.ReportID == reportID && body.Status == "processing" &&
					(body.Code == "" || body.Code == "traffic_report_processing") {
					return &TrafficReportProcessingError{
						ReportID:   reportID,
						RetryAfter: trafficRetryAfter(response.Header().Get("Retry-After")),
					}
				}
				if body.Code == "traffic_report_payload_changed" ||
					(body.Code == "" && body.Error == "Traffic report payload changed") {
					return &TrafficReportPayloadConflictError{ReportID: reportID}
				}
			}
		}
...
func trafficRetryAfter(value string) time.Duration {
