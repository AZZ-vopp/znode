...
// UpdateFallbackConfig changes the signed Redis user-snapshot source. The node
// controller updates the matching live device limiter separately before
// publishing this client-side view; neither operation rebuilds Xray inbounds.
...
func New(c *conf.NodeConfig) (*Client, error) {
	...
	client.SetHeader("X-ZNode-Capabilities", "default_wg_balancer")
	if c.Timeout > 0 {
