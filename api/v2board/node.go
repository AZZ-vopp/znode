...
// controlPlaneHTTPError preserves the panel response class so callers can
// distinguish a panel availability outage from credentials or bad config.
...
func (e *controlPlaneHTTPError) Error() string {
	return fmt.Sprintf("%s: panel returned HTTP %d", e.operation, e.status)
}

// IsControlPlaneAvailabilityError reports only failures for which the ZBoard
// control-plane is unavailable. Authentication, redirects, malformed
// responses and other policy/configuration errors must not relax admission.
func IsControlPlaneAvailabilityError(err error) bool {
	if err == nil || errors.Is(err, context.Canceled) {
		return false
	}
	var response *controlPlaneHTTPError
	if errors.As(err, &response) {
		return isControlPlaneAvailabilityStatus(response.status)
	}
	var userListResponse *userListHTTPError
	if errors.As(err, &userListResponse) {
		return isControlPlaneAvailabilityStatus(userListResponse.Status)
	}
...
func isControlPlaneAvailabilityStatus(status int) bool {
	switch status {
...
type CommonNode struct {
	...
	// DeviceLimitExcludedIPs contains exact relay/proxy source IPs which must
	// not consume a customer's device allowance or appear in device reports.
	// It is deliberately per logical node: a relay address on one ingress is
	// not necessarily trusted by another one.
	DeviceLimitExcludedIPs []string    `json:"device_limit_excluded_ips,omitempty"`
...
func (c *Client) GetNodeInfo(ctx context.Context) (node *NodeInfo, err error) {
	...
	if r.StatusCode() < 200 || r.StatusCode() >= 300 {
		return nil, &controlPlaneHTTPError{operation: "get node config", status: r.StatusCode()}
	}
	...
	node.Common = cm
	// Cache only a fully decoded and validated response. Otherwise an identical
	// malformed 200 on the next poll would look like an unchanged healthy
	// response and could incorrectly end an outage bypass.
