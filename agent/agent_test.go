...
func TestMonitorHotSwapsRedisFallbackWithoutReloadingNodes(t *testing.T) {
	reloadCh := make(chan struct{}, 1)
	...
	select {
	case <-reloadCh:
		t.Fatal("Redis fallback change unexpectedly reloaded VPN nodes")
	default:
	}
	select {
	case update := <-fallbackCh:
		if update.Revision != "fallback-2" || update.AggregateRevision != "all-2" || update.Config == nil ||
			update.Config.RedisAddr != "redis.example:6379" || !update.Config.UserFallbackEnabled {
			t.Fatalf("unexpected Redis hot-swap payload: %+v", update)
		}
	default:
		t.Fatal("Redis fallback change was not hot-swapped")
	}
	...
	select {
	case <-reloadCh:
		t.Fatal("same Redis fallback revision unexpectedly reloaded VPN nodes")
	default:
	}
	select {
	case update := <-fallbackCh:
		t.Fatalf("same Redis fallback revision was applied twice: %+v", update)
	default:
	}
}
