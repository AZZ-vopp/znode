...
const (
	deviceRefreshCooldown        = 5 * time.Second
	defaultMaxTrackedCredentials = 65536
	defaultMaxTrackedEntries     = 262144
)
...
func (t *deviceTracker) copyRuntimeStateFrom(source *deviceTracker) {
	if t == nil || source == nil || t == source {
		return
	}
	source.mu.Lock()
	t.mu.Lock()
	...
	all := make([]runtimeEntry, 0, source.totalEntries)
	for userKey, entries := range source.users {
		for ip, entry := range entries {
			if entry != nil {
				all = append(all, runtimeEntry{userKey: userKey, ip: ip, entry: entry})
			}
		}
	}
	// A lower hot-swapped bound retains the most recently active state first.
	sort.Slice(all, func(i, j int) bool { return all[i].entry.lastSeen.After(all[j].entry.lastSeen) })
	for _, item := range all {
		entries := t.users[item.userKey]
		if entries == nil {
			if len(t.users) >= t.maxCredentials || t.totalEntries >= t.maxEntries {
				continue
			}
			entries = make(map[string]*deviceEntry)
			t.users[item.userKey] = entries
		}
		if t.totalEntries >= t.maxEntries || len(entries) >= t.maxIPsPerUser {
			continue
		}
		copyEntry := *item.entry
		copyEntry.remoteInFlight = false
		entries[item.ip] = &copyEntry
		t.totalEntries++
	}
	t.mu.Unlock()
	source.mu.Unlock()

	source.aliveMu.RLock()
	alive := make(map[int]int, len(source.alive))
	for uid, count := range source.alive {
		alive[uid] = count
	}
	source.aliveMu.RUnlock()
	t.SetAliveList(alive)
}
...
func newDeviceTracker(c *conf.GlobalDeviceLimitConfig) *deviceTracker {
	t := &deviceTracker{
		...
		maxIPsPerUser:       256,
		maxIPsPerCredential: 1,
		maxCredentials:      defaultMaxTrackedCredentials,
		maxEntries:          defaultMaxTrackedEntries,
	}
	if c != nil {
		...
		t.maxIPsPerCredential = c.MaxIPsPerCredential
...
// effectiveLimit is the number of source IP aliases admitted for a single
// credential. User/device identity is the UUID key; mobile CGNAT may expose
// several public IPv4 addresses for that same key. Multiplying a larger panel
// limit preserves the meaning of a multi-device allowance while retaining the
// configured per-credential fanout cap.
func (t *deviceTracker) effectiveLimit(limit int) int {
	if limit <= 0 || t.maxIPsPerCredential <= 1 {
		return limit
	}
	if limit > int(^uint(0)>>1)/t.maxIPsPerCredential {
		return int(^uint(0) >> 1)
	}
	return limit * t.maxIPsPerCredential
}
...
func (t *deviceTracker) Observe(ctx context.Context, remote deviceStore, failClosed bool, userKey, ip string, uid, limit int, now time.Time) (bool, error) {
	...
	if entries != nil {
		t.pruneLocked(userKey, entries, now)
		entries = t.users[userKey]
	}
	entry, exists := (*deviceEntry)(nil), false
	if entries != nil {
	...
	if !exists {
		if !t.hasCapacityLocked(entries == nil, now) {
			// A limited credential cannot be admitted without recording the
			// decision. Unlimited traffic stays available but deliberately does
			// not consume more memory or appear in online-device reports.
			t.mu.Unlock()
			return limit <= 0, nil
		}
		if entries == nil {
...
func (t *deviceTracker) completeRefresh(userKey, ip string, allowed bool, err error) {
	...
	t.deleteEntryLocked(userKey, entries, ip)
}
...
func (t *deviceTracker) Delete(userKey string) {
	t.mu.Lock()
	t.deleteUserLocked(userKey)
	t.mu.Unlock()
}

func (t *deviceTracker) Clear() {
	t.mu.Lock()
	t.users = make(map[string]map[string]*deviceEntry)
	t.totalEntries = 0
	t.mu.Unlock()
}
...
func (t *deviceTracker) evictHandoverLocked(entries map[string]*deviceEntry, limit int, now time.Time) bool {
	...
	for i := 0; i < needed; i++ {
		if _, exists := entries[candidates[i]]; exists {
...
func (t *deviceTracker) pruneLocked(userKey string, entries map[string]*deviceEntry, now time.Time) {
...
// hasCapacityLocked performs a full TTL sweep only under global pressure, so
// the ordinary packet path remains O(1). It never evicts a live limited entry.
func (t *deviceTracker) hasCapacityLocked(newCredential bool, now time.Time) bool {
	if (!newCredential || len(t.users) < t.maxCredentials) && t.totalEntries < t.maxEntries {
		return true
	}
	for userKey, entries := range t.users {
		t.pruneLocked(userKey, entries, now)
	}
	return (!newCredential || len(t.users) < t.maxCredentials) && t.totalEntries < t.maxEntries
}

func (t *deviceTracker) deleteEntryLocked(userKey string, entries map[string]*deviceEntry, ip string) {
...
func (t *deviceTracker) deleteUserLocked(userKey string) {
	if entries := t.users[userKey]; entries != nil {
		t.totalEntries -= len(entries)
		delete(t.users, userKey)
	}
}
