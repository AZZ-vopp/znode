...
func (r *redisDeviceStore) DeleteMany(ctx context.Context, userKeys []string) error {
	if r == nil || len(userKeys) == 0 {
		return nil
	}
	requestCtx, cancel := context.WithTimeout(ctx, r.timeout)
	defer cancel()
	keys := make([]string, 0, len(userKeys)*2)
	for _, userKey := range userKeys {
		keys = append(keys, r.key(userKey), r.handoverKey(userKey))
	}
	return r.client.Unlink(requestCtx, keys...).Err()
}

func (r *redisDeviceStore) ClearAll(ctx context.Context) error {
	if r == nil {
		return nil
	}
	timeout := r.timeout
	...
	defer cancel()
	pattern := r.key("")
	pattern = pattern[:strings.LastIndex(pattern, ":")+1] + "*"
	var cursor uint64
	for {
		keys, next, err := r.client.Scan(requestCtx, cursor, pattern, 1000).Result()
		if err != nil {
			return err
		}
		if len(keys) > 0 {
			if err := r.client.Unlink(requestCtx, keys...).Err(); err != nil {
				return err
			}
		}
		cursor = next
