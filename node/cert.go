...
var certificateWriteRoots = []string{"/etc/znode"}
...
func (c *Controller) requestCertContext(ctx context.Context) error {
	...
	writable := cert.CertMode == "dns" || cert.CertMode == "http" || cert.CertMode == "auto" || cert.CertMode == "self" || cert.CertMode == "remote"
	if writable {
	...
	case "remote":
		...
		if cert.TlsCert == "" && cert.TlsKey == "" && file.IsExist(cert.CertFile) && file.IsExist(cert.KeyFile) {
			return nil
		}
		if cert.TlsCert == "" || cert.TlsKey == "" {
			return fmt.Errorf("tls cert or tls key not exist")
		}
		if err := writeRemoteCertificatePair(cert.CertFile, cert.KeyFile, []byte(cert.TlsCert), []byte(cert.TlsKey)); err != nil {
			return fmt.Errorf("write remote certificate pair: %w", err)
		}
...
// writeRemoteCertificatePair validates panel-provided material before touching
// the live files, stages both files durably, and retains the previous pair
// until both renames succeed. The per-pair lock serializes certificate writers;
// each individual replacement is an atomic rename.
func writeRemoteCertificatePair(certPath, keyPath string, certPEM, keyPEM []byte) error {
	if len(certPEM) == 0 || int64(len(certPEM)) > maxCertificateMaterialBytes ||
		len(keyPEM) == 0 || int64(len(keyPEM)) > maxCertificateMaterialBytes {
		return fmt.Errorf("certificate material must be between 1 byte and %d bytes", maxCertificateMaterialBytes)
	}
	if _, err := tls.X509KeyPair(certPEM, keyPEM); err != nil {
		return fmt.Errorf("invalid or mismatched certificate/key PEM: %w", err)
	}
	...
	// Re-check after directory creation and immediately before staging. This
	// rejects a symlink in either the final file or any existing parent.
	if err := validateCertificateMaterialPath(certPath, false, true); err != nil {
		return err
	}
	if err := validateCertificateMaterialPath(keyPath, true, true); err != nil {
		return err
	}

	certTemp, err := stageCertificateFile(certPath, certPEM, 0o644)
	if err != nil {
		return fmt.Errorf("stage certificate: %w", err)
	}
	defer os.Remove(certTemp)
	keyTemp, err := stageCertificateFile(keyPath, keyPEM, 0o600)
	if err != nil {
		return fmt.Errorf("stage private key: %w", err)
	}
	defer os.Remove(keyTemp)

	certBackup, err := backupCertificateFile(certPath)
	if err != nil {
		return fmt.Errorf("backup certificate: %w", err)
	}
	keyBackup, err := backupCertificateFile(keyPath)
	if err != nil {
		restoreCertificateFile(certPath, certBackup)
		return fmt.Errorf("backup private key: %w", err)
	}
	committedCert, committedKey := false, false
	rollback := func() {
		if committedCert {
		...
		restoreCertificateFile(certPath, certBackup)
		restoreCertificateFile(keyPath, keyBackup)
	}
	...
	committedCert = true
	if err := syncCertificateDirectories(certPath, keyPath); err != nil {
		rollback()
...
func stageCertificateFile(path string, content []byte, mode os.FileMode) (string, error) {
	temporary, err := os.CreateTemp(filepath.Dir(path), ".znode-cert-stage-*")
	if err != nil {
		return "", err
	}
...
func backupCertificateFile(path string) (string, error) {
	if _, err := os.Lstat(path); os.IsNotExist(err) {
		return "", nil
	} else if err != nil {
		return "", err
	}
	placeholder, err := os.CreateTemp(filepath.Dir(path), ".znode-cert-backup-*")
	if err != nil {
		return "", err
	}
...
func restoreCertificateFile(path, backup string) {
	if backup != "" {
