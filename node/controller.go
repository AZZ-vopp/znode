...
type Controller struct {
	...
	appliedDeviceIPClears      map[string]struct{}
	appliedDeviceIPClearOrder  []string
	deviceIPClearJournalLoaded bool
	nodeConfigFailures         atomic.Int32
	inboundActive              bool
	prepared                   bool
	started                    bool
}

// closeDrainAttempts gives already-interrupted sessions a few short chances
// to publish their final counters before a reload is rolled back. We retain
// the quiesce barrier between attempts, so no new session can enter and no
// traffic is discarded merely because the first five-second window elapsed.
const closeDrainAttempts = 3
...
type controllerLimiter interface {
	...
	SetDeviceLimitBypass(bool) bool
}

// SetDeviceLimitBypass changes only the device/IP admission policy. The
// health monitor calls it after a node-config request outcome; it never asks
// Xray to reload, so established sessions stay untouched.
func (c *Controller) SetDeviceLimitBypass(bypass bool) {
	c.userSyncMu.Lock()
	if c.deviceLimitBypass.Load() == bypass {
		c.userSyncMu.Unlock()
		return
	}
	c.deviceLimitBypass.Store(bypass)
	l := c.limiter
	if l != nil {
		l.SetDeviceLimitBypass(bypass)
	}
	c.userSyncMu.Unlock()
	entry := log.WithField("tag", c.tag)
	if bypass {
		entry.Warn("Panel unavailable; device/IP limiting temporarily bypassed")
		return
	}
	entry.Info("Panel recovered; device/IP limiting re-enabled")
}
...
func (c *Controller) UpdateFallbackConfig(config *conf.GlobalDeviceLimitConfig) {
	...
	nextWatcher := newDeviceSyncWatcher(cloned, c.conf.APIHost)
	if nextWatcher != nil {
		if err := nextWatcher.Start(c.conf.APIHost, c.handleDeviceSyncEvent); err != nil {
			log.WithError(err).WithField("tag", c.tag).Warn("Updated device UUID fast-sync disabled")
			nextWatcher = nil
		}
	}

	c.userSyncMu.Lock()
	oldWatcher := c.deviceSyncWatcher
	if updater, ok := c.limiter.(interface {
		UpdateDeviceConfig(*conf.GlobalDeviceLimitConfig, string) error
	}); ok {
		if err := updater.UpdateDeviceConfig(cloned, c.conf.APIHost); err != nil {
			log.WithError(err).WithField("tag", c.tag).Warn("Redis device limiter hot-swap fell back to configured availability policy")
		}
	}
	c.applyDeviceIPClearCommandsLocked(cloned)
	c.conf.GlobalDeviceLimitConfig = cloned
	c.apiClient.UpdateFallbackConfig(cloned)
	c.deviceSyncWatcher = nextWatcher
	c.userSyncMu.Unlock()
	if oldWatcher != nil {
		oldWatcher.Close()
	}
...
func (c *Controller) Prepare(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get user alive list: %w", err)
	}
...
func (c *Controller) Start(x *core.V2Core) error {
	...
	if c.info != nil && c.info.Common != nil {
		deviceLimitExcludedIPs = c.info.Common.DeviceLimitExcludedIPs
	}
	l := limiter.AddLimiter(c.info.Type, c.tag, runtimeUsers, c.aliveMap, c.conf.GlobalDeviceLimitConfig, c.conf.APIHost, deviceLimitExcludedIPs)
	c.userSyncMu.Lock()
	if c.deviceLimitBypass.Load() {
		l.SetDeviceLimitBypass(true)
	}
	c.limiter = l
	c.applyDeviceIPClearCommandsLocked(c.conf.GlobalDeviceLimitConfig)
	c.userSyncMu.Unlock()
...
func (c *Controller) Close() error {
	...
	if err := c.drainNodeLinks(context.Background(), false); err != nil {
		return c.restoreAfterCloseFailureLocked(fmt.Errorf("drain node links [%s]: %w", c.tag, err))
	}
...
func (c *Controller) Shutdown(ctx context.Context) error {
	...
	} else if err := c.drainNodeLinks(ctx, true); err != nil {
		shutdownErr = errors.Join(shutdownErr, fmt.Errorf("drain node links [%s]: %w", c.tag, err))
	}
	if err := ctx.Err(); err != nil {
...
// drainNodeLinks retries only the bounded drain operation. A timeout means
// the tag is already quiesced and all links have already been interrupted;
// retrying is therefore safe and allows a slow transport to finish its final
// accounting read without reopening admission or losing its byte count.
func (c *Controller) drainNodeLinks(ctx context.Context, terminal bool) error {
	var drainErr error
	for attempt := 1; attempt <= closeDrainAttempts; attempt++ {
		drainErr = c.coreExecutor().quiesceNodeLinks(ctx, terminal, c.tag)
		if drainErr == nil {
			return nil
		}
		...
		if attempt < closeDrainAttempts {
			log.WithFields(log.Fields{
				"tag":     c.tag,
				"attempt": attempt,
				"retries": closeDrainAttempts - attempt,
				"err":     drainErr,
			}).Warn("Traffic drain did not finish; retrying before runtime reload")
		}
	}
	return drainErr
}
...
func (c *Controller) startBackgroundServices() {
	...
	if c.deviceSyncWatcher != nil {
		if err := c.deviceSyncWatcher.Start(c.conf.APIHost, c.handleDeviceSyncEvent); err != nil {
			c.deviceSyncWatcher = nil
