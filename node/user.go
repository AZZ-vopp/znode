...
func (c *Controller) reportUserTrafficTask(ctx context.Context) (err error) {
	...
	c.userSyncMu.Unlock()
	if err != nil {
		fields := log.Fields{"tag": c.tag, "err": err}
		var processingError *panel.TrafficReportProcessingError
		var conflictError *panel.TrafficReportPayloadConflictError
		switch {
		case errors.As(err, &processingError):
			if processingError.RetryAfter > 0 {
				fields["retry_after"] = processingError.RetryAfter
			}
			log.WithFields(fields).Info("Traffic report is still processing; immutable batch retained for retry")
		case errors.As(err, &conflictError):
			log.WithFields(fields).Error("Traffic report ID conflicts with panel payload; immutable batch retained")
		default:
			log.WithFields(fields).Info("Report user traffic failed; batch retained for retry")
		}
