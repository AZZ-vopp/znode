...
// SetDeviceLimitBypass forwards panel health to every assigned controller.
// The flag is retained before Start so a runtime restored from snapshot starts
// permissive for device/IP count until the first successful panel poll.
func (n *Node) SetDeviceLimitBypass(bypass bool) {
	if n == nil {
		return
	}
	for _, controller := range n.controllers {
		if controller != nil {
			controller.SetDeviceLimitBypass(bypass)
		}
	}
}

// DeviceLimitBypassed reports the retained controller state. It is primarily
// useful to verify an offline prepared runtime before its limiters exist.
func (n *Node) DeviceLimitBypassed() bool {
	if n == nil || len(n.controllers) == 0 {
		return false
	}
	for _, controller := range n.controllers {
		if controller == nil || !controller.deviceLimitBypass.Load() {
			return false
		}
	}
	return true
...
func cloneDeviceConfig(source *conf.GlobalDeviceLimitConfig) *conf.GlobalDeviceLimitConfig {
	...
	cloned.ClearCommands = cloneDeviceIPClearCommands(source.ClearCommands)
...
func cloneDeviceIPClearCommands(source []conf.DeviceIPClearCommand) []conf.DeviceIPClearCommand {
	cloned := make([]conf.DeviceIPClearCommand, len(source))
	for i := range source {
		cloned[i] = source[i]
		cloned[i].CredentialHashes = append([]string(nil), source[i].CredentialHashes...)
	}
	return cloned
}
