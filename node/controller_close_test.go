...
type retryDrainCore struct {
	controllerCore
	calls atomic.Int32
}

func (*retryDrainCore) removeNode(context.Context, string) error { return nil }

func (c *retryDrainCore) quiesceNodeLinks(context.Context, string) error {
	if c.calls.Add(1) < closeDrainAttempts {
		return errors.New("timed out waiting for traffic reads to drain")
	}
	return nil
}

func (*retryDrainCore) captureUserTraffic(context.Context, string, int) (*vcore.UserTrafficCapture, error) {
	return &vcore.UserTrafficCapture{Snapshot: map[int]int64{}}, nil
}

func TestCloseRetriesTrafficDrainBeforeRollingBack(t *testing.T) {
	backend := &retryDrainCore{}
	controller := &Controller{
		server:             &vcore.V2Core{},
		tag:                "retry-drain",
		info:               &panel.NodeInfo{Id: 93, Tag: "retry-drain"},
		started:            true,
		inboundActive:      true,
		trafficSpoolLoaded: true,
	...
	if err := controller.Close(); err != nil {
		t.Fatalf("close with transient drain timeouts: %v", err)
	}
	if got := backend.calls.Load(); got != closeDrainAttempts {
		t.Fatalf("drain attempts = %d, want %d", got, closeDrainAttempts)
	}
}
...
func (*blockingOnlineLimiter) SetDeviceLimitBypass(bool) bool { return false }
