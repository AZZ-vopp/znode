...
// settings share one Pub/Sub connection; each subscriber only keeps a small,
// bounded notification channel. This prevents Redis clients and reconnect
...
type deviceSyncSubscriber struct {
	apiHost    string
	signingKey string
	handle     func(deviceSyncEvent)
	notify     chan deviceSyncEvent
	stop       chan struct{}
	done       chan struct{}
	once       sync.Once
}
...
func (w *deviceSyncWatcher) Start(apiHost string, handle func(deviceSyncEvent)) error {
	if w == nil || handle == nil {
		return nil
	}
	w.mu.Lock()
	...
	w.hubKey = key
	w.id = hub.addSubscriber(host, w.config.SyncSigningKey, handle)
	hub.Start()
	w.started = true
	return nil
}
...
func (h *deviceSyncHub) addSubscriber(apiHost, signingKey string, handle func(deviceSyncEvent)) uint64 {
	id := deviceSyncSubscriberSequence.Add(1)
	subscriber := &deviceSyncSubscriber{
		apiHost:    normalizeDeviceSyncAPIHost(apiHost),
		signingKey: strings.TrimSpace(signingKey),
		handle:     handle,
		notify:     make(chan deviceSyncEvent, 16),
		stop:       make(chan struct{}),
		done:       make(chan struct{}),
	}
	h.mu.Lock()
...
func (s *deviceSyncSubscriber) signal(event deviceSyncEvent) {
	select {
	case s.notify <- event:
	default:
	}
...
func (h *deviceSyncHub) dispatch(payload string) {
	...
	// A connection-IP clear changes admission state, so it must always be
	// explicitly scoped to one panel and carry the versioned idempotency token.
	// Legacy device notifications may remain unscoped for compatibility.
	...
	for _, subscriber := range h.subscribers {
		if (eventHost == "" || subscriber.apiHost == eventHost) &&
			(event.Action != "device.connection_ips_cleared" || validDeviceIPClearEventSignature(event, subscriber.signingKey, time.Now())) {
...
func validDeviceIPClearEventSignature(event deviceSyncEvent, signingKey string, now time.Time) bool {
	if len(signingKey) != sha256.Size*2 || event.At <= 0 || event.At > now.Unix()+300 || now.Unix()-event.At > 600 {
		return false
	}
	if decoded, err := hex.DecodeString(signingKey); err != nil || len(decoded) != sha256.Size {
		return false
	}
...
func deviceIPClearSignatureInput(event deviceSyncEvent) string {
	clearAll := "0"
	if event.ClearAll {
		clearAll = "1"
	}
	return strings.Join([]string{
		...
		event.Generation,
		clearAll,
		strings.Join(event.CredentialHashes, ","),
		strconv.FormatInt(event.At, 10),
	}, "\n")
}
