...
func serverHandle(_ *cobra.Command, _ []string) {
	...
	relayCh := make(chan agent.RelayUpdate, 1)
	relaySupervisor := relay.NewSupervisor(context.Background())
	if err := relaySupervisor.Reconcile(prepared.assignment.UDPRelays); err != nil {
		log.WithField("err", err).Error("Start managed UDP relays failed")
		return
	}
	...
	agentMonitor.SetRelayChannel(relayCh)
	...
	for {
		...
		case <-reloadCh:
			...
			previousRelays := running.assignment.UDPRelays
			previousRelayRevision := running.assignment.UDPRelayRevision
			...
			running = newRuntime
			if err := relaySupervisor.Reconcile(running.assignment.UDPRelays); err != nil {
				// Node reload succeeded, but leave the relay component pending and
				// preserve its last-known-good assignment for the next retry.
				log.WithField("err", err).Error("Managed UDP relay reload failed; keeping prior relays")
				running.assignment.UDPRelays = previousRelays
				running.assignment.UDPRelayRevision = previousRelayRevision
			}
			if err := persistRuntimeSnapshot(running.preparedRuntime); err != nil {
				log.WithField("err", err).Warn("Persist last-known-good runtime snapshot failed")
			}
		...
		case update := <-relayCh:
			if err := relaySupervisor.Reconcile(update.Configs); err != nil {
				// Keep prior relays.  The next successful manifest poll retries this
				// revision because it is intentionally not marked as applied here.
				log.WithField("err", err).Error("Managed UDP relay update failed; keeping current relays")
				continue
			}
			running.assignment.UDPRelays = update.Configs
			running.assignment.UDPRelayRevision = update.Revision
			agentMonitor.MarkRelayApplied(update.Revision)
			if err := persistRuntimeSnapshot(running.preparedRuntime); err != nil {
				log.WithField("err", err).Warn("Persist UDP relay snapshot failed")
			}
			log.WithField("relays", len(update.Configs)).Info("Applied managed UDP relays")
		case <-snapshotCh:
			if err := persistRuntimeSnapshot(running.preparedRuntime); err != nil {
