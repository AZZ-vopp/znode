...
type runtimeSnapshotPayload struct {
	...
	UDPRelays            []relay.Config       `json:"udp_relays,omitempty"`
...
func prepareInitialRuntime(configPath string) (*preparedRuntime, error) {
	...
	// A snapshot proves the last known configuration but not current panel
	// reachability. Relax only a genuine availability failure; credentials,
	// TLS, redirects or invalid responses must not open device admission.
	if panel.IsControlPlaneAvailabilityError(onlineErr) {
		prepared.nodes.SetDeviceLimitBypass(true)
	}
	return prepared, nil
}
...
func prepareRuntimeFromSnapshot(configPath string) (*preparedRuntime, error) {
	...
	for _, relayConfig := range payload.UDPRelays {
		if err := relayConfig.Validate(); err != nil {
			return nil, fmt.Errorf("offline runtime relay assignment is invalid: %w", err)
		}
	}
	...
	return &preparedRuntime{
		...
		assignment: agent.Assignment{
			...
			UDPRelayRevision:     payload.UDPRelayRevision,
			UDPRelays:            payload.UDPRelays,
...
func persistRuntimeSnapshot(prepared *preparedRuntime) error {
	...
	payload := runtimeSnapshotPayload{
		...
		UDPRelayRevision:     prepared.assignment.UDPRelayRevision,
		UDPRelays:            prepared.assignment.UDPRelays,
