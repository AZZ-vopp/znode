...
const LegacyDefaultMaxConnectionsPerUser = 128
const DefaultMaxConnectionsPerUser = 512
const DefaultMaxConnections = 32768
const MaximumIPsPerUser = 1024
const MaximumIPsPerCredential = 64
...
type GlobalDeviceLimitConfig struct {
	...
	// MaxIPsPerCredential allows one credential to use several public IPs at
	// once. Mobile carrier CGNAT can rotate the egress IPv4 per connection, so
	// counting every IP as a separate device disconnects a single hotspot user.
	// The credential/UUID remains the device identity; this is only an IP alias
	// allowance. Set to 1 to restore strict one-IP admission.
	MaxIPsPerCredential int    `mapstructure:"MaxIPsPerCredential"`
	...
	// SyncSigningKey authenticates high-impact Redis control events. It is a
	// domain-separated key delivered by the Agent manifest, never APP_KEY.
	SyncSigningKey string `mapstructure:"SyncSigningKey"`
	...
	// ClearCommands is a bounded, replayable control-plane log. It makes an
	// Admin IP reset survive a dropped Pub/Sub event or a temporarily offline
	// Agent without exposing raw credential UUIDs.
	ClearCommands []DeviceIPClearCommand `mapstructure:"ClearCommands"`
}

type DeviceIPClearCommand struct {
	// Version, Action, APIHost, At and Signature bind this retained command to
	// one authenticated control plane. Unlike ephemeral Redis events, durable
	// commands can be replayed after an Agent restart and must fail closed when
	// any of those fields are absent or invalid.
	Version          int      `mapstructure:"Version"`
	APIHost          string   `mapstructure:"APIHost"`
	Action           string   `mapstructure:"Action"`
	Generation       string   `mapstructure:"Generation"`
	ClearAll         bool     `mapstructure:"ClearAll"`
	CredentialHashes []string `mapstructure:"CredentialHashes"`
	At               int64    `mapstructure:"At"`
...
func (c *ConnectionConfig) applyDefaults() {
	...
	if c.MaxConnectionsPerUser <= 0 || c.MaxConnectionsPerUser == LegacyDefaultMaxConnectionsPerUser {
...
func (c *GlobalDeviceLimitConfig) applyDefaults() {
	...
	if c.MaxIPsPerUser > MaximumIPsPerUser {
		c.MaxIPsPerUser = MaximumIPsPerUser
	}
	if c.MaxIPsPerCredential <= 0 {
		c.MaxIPsPerCredential = 4
	}
	if c.MaxIPsPerCredential > MaximumIPsPerCredential {
		c.MaxIPsPerCredential = MaximumIPsPerCredential
	}
