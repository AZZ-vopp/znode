...
func TestTaskTimeoutKeepsRuntimeInsteadOfRequestingReload(t *testing.T) {
	reloadCh := make(chan struct{}, 1)
	canceled := make(chan struct{})
	task := &Task{
		Name:     "panel-timeout-test",
	...
	if err := task.ExecuteWithTimeout(); err != nil {
		t.Fatalf("context-aware timeout returned an error: %v", err)
	}
	select {
	case <-canceled:
	default:
		t.Fatal("task callback did not observe timeout cancellation")
	}
	select {
	case <-reloadCh:
		t.Fatal("control-plane task timeout requested a full runtime reload")
	default:
	}
}
