...
func (t *Task) executeWithTimeout(stop <-chan struct{}) error {
	...
	case <-ctx.Done():
		// A control-plane timeout must not recycle every VPN listener. The
		// periodic worker remains alive and retries after its normal interval;
		// the running core continues serving its last-known-good state.
		log.Errorf("Task %s execution timed out; keeping runtime and retrying", t.Name)
		return waitForTaskCancellation(t.Name, done)
	case err := <-done:
